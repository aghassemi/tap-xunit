...
function converter(options) {
  ...
  function formatFailure(diag) {
    var text = '\n          ---\n';
    if(diag.operator !== undefined) {
      text += '            operator: ' + diag.operator + '\n';
    }
    if(diag.expected !== undefined) {
      text += '            expected: ' + (typeof diag.expected === 'object' ? JSON.stringify(diag.expected) : diag.expected) + '\n';
    }
    if(diag.actual !== undefined) {
      text += '            actual:   ' + (typeof diag.actual === 'object' ? JSON.stringify(diag.actual) : diag.actual)  + '\n';
    }
    if(diag.at !== undefined) {
      text += '            at: ' + diag.at + '\n';
    }
    if(diag.stack !== undefined) {
      text += '            stack: ' + diag.stack + '\n';
    }
    text += '          ...\n      ';
